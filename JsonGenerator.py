...
import pandas as pd
import json, os, re ,subprocess, sys , platform
...
def install_requirements():
    ...
    subprocess.check_call(
        [pip_exec, "install", "-r", "requirements.txt", "-q"],
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL
    )
    #subprocess.check_call([pip_exec, "install", "-r", "requirements.txt"]) # If this subproccess is enabled (instead of the other one)
                                                                            # user will be able to see the warnings and errors in console.
                                                                            # Is useful for the developer. 
...
if __name__ == "__main__":
     # Only execute virtual env if we are out of it (otherwise we call it in eternal loop)
    if sys.prefix == sys.base_prefix:
        if not venv_exists():
            create_venv()
        install_requirements()
        run_main_script()
        sys.exit()  # Needed for following with the script in the first process
    ...
    for sheet_name, df in df_filtered_dict.items():
        ...
        print(f"\n {sheet_name}")
        print("*"*(len(f"* JSON file ../JSON FILES/{sheet_name}/aa.json has been created *")))
